...
if (isset($_GET['content_type'])) {
    ...
    $posts_sql .= " WHERE content_type_id = ? ORDER BY views_count DESC LIMIT ? OFFSET ?";
    ...
    $posts_sql .= " ORDER BY views_count DESC LIMIT ? OFFSET ?";
    $stmt = mysqli_prepare($con, $posts_sql);
...
for ($i = 0; $i < count($posts); $i++) {
    $posts[$i]['likes'] = get_num_likes($con, $posts[$i]['id']);
    $posts[$i]['is_like'] = is_like($con, $posts[$i]['id'], $_SESSION['user_id']);
    $posts[$i]['comments'] = get_num_comments($con, $posts[$i]['id']);
}
